//check objects and put var for local ones
 ...
 var nums=[];
 var tableSize;
 var img = 'https://www.zahratalkhaleej.ae/assets/images/Articles/1140x635/2019/7/20197231247427256L.jpg';
//  './animal-1.jpg'
 // 
...
function initlization(){
	...
	myTable.addEventListener("dragenter", function(e){e.preventDefault();}, false);
	myTable.addEventListener("dragover", function(e){e.preventDefault();}, false);
...
function shuffle(){
    ...
    for(var i=0;i<tableSize;i++){
        ...
        // }
        blocks[i].innerHTML='<span id=' + i + ' class="puzzle-peice" draggable="true"> </span>';

        if( i!==0 && i%(blocks.length/rows.length) === 0 ) {
            yPosition = ++rowNumber * 300;
        }
        blocks[i].children[0].style="background-image: url(" +img + ");background-position:" + -i%(blocks.length/rows.length) *300 +  "px " + -1*yPosition + "px";
    } 
    shuffleMyCells();
}
...
function playAction (e){
        ...
        if(clickedBlock.tagName!="TD")//if first its not a td objects
        {
...
function isAdjacent(){
    ...
    var targetY=empty.parentElement.rowIndex;
     ...
     return true;
    else if ((x+1)==targetX && y==targetY)
     return true;
    else if (x==targetX && (y-1)==targetY)
     return true;
    else if (x==targetX && (y+1)==targetY)
     return true;
    else 
